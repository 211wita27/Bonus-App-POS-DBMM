...
CREATE TABLE loyalty_account (
    ...
    CONSTRAINT chk_loyalty_account_current_points CHECK (current_points >= 0)
);
...
CREATE TABLE point_rule (
    ...
    CONSTRAINT chk_point_rule_multiplier CHECK (multiplier >= 0),
    CONSTRAINT chk_point_rule_amount_threshold CHECK (amount_threshold >= 0),
    CONSTRAINT chk_point_rule_base_points CHECK (base_points >= 0)
);
...
CREATE TABLE reward (
    ...
    CONSTRAINT chk_reward_cost_points CHECK (cost_points > 0)
);
...
CREATE TABLE purchase (
    ...
    CONSTRAINT chk_purchase_total_amount CHECK (total_amount > 0),
    CONSTRAINT chk_purchase_currency CHECK (length(currency) = 3)
);
...
CREATE TABLE redemption (
    ...
    CONSTRAINT fk_redemption_branch FOREIGN KEY (branch_id) REFERENCES branch (id),
    CONSTRAINT uk_redemption_ledger UNIQUE (ledger_entry_id),
    CONSTRAINT chk_redemption_points_spent CHECK (points_spent > 0)
);
...
CREATE INDEX idx_redemption_branch ON redemption (branch_id);
